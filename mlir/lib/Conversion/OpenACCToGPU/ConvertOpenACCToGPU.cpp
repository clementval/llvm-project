...
static FuncOp outlineParallelRegion(acc::ParallelOp parallelOp,
  ...
  for (Value v : allOperands) {
    ...
    for (Value p : parallelOp.gangPrivateOperands()) {
      if (v == p) {
        isInPrivate = true;
        privates.insert(v);
        break;
...
// the forOp.
static LogicalResult collapseNestedLoops(scf::ForOp rootForOp,
...
static LogicalResult applyCollapseClause(acc::LoopOp accLoopOp) {
  if (!accLoopOp.hasCollapseAttr())
    return success();
  if (auto forOp = dyn_cast<scf::ForOp>(accLoopOp.getBody().front().front())) {
...
static void hoistOpBeforeOperation(StructureOp &parentOp,
  ...
  for (auto &op : accLoopOp.getBody().front().getOperations()) {
    if (isa<scf::ForOp>(op) || isa<AffineForOp>(op))
      break;
    toHoist.push_back(&op);
  }
...
static scf::IfOp createGangRedundantWrapper(OpBuilder &builder, Location loc,
  ...
  auto ifOp = builder.create<scf::IfOp>(loc, isThread0, false);
  ...
  if (auto forOp = dyn_cast<scf::ForOp>(accLoopOp.getBody().front().front())) {
    ...
    if (accLoopOp.isSeq()) {
      if (accLoopOp.isGangRedundant()) {
        scf::IfOp wrapper = createGangRedundantWrapper(builder, loc, indexOps);
      ...
      dyn_cast<AffineForOp>(accLoopOp.getBody().front().front())) {
    OpBuilder builder(forOp);
    if (accLoopOp.isSeq()) {
      if (accLoopOp.isGangRedundant()) {
        scf::IfOp wrapper = createGangRedundantWrapper(builder, loc, indexOps);
    ...
    forOp.emitError("affine.for not handled yet.");
  }
...
static void transformGangRedundant(acc::GangRedundantOp accGangRedundantOp,
  ...
  wrapper.walk([&](acc::GangRedundantEndOp op) {
    auto yield = builder.create<scf::YieldOp>(loc);
...
static void applyGangPrivateList(gpu::GPUFuncOp outlinedParallelRegion,
                                 ...
                                 llvm::SetVector<Value> &privates) {
  if (accParallelOp.gangPrivateOperands().size() == 0)
    return;

  assert(accParallelOp.gangPrivateOperands().size() == privates.size() &&
         "Number of private variable doesn't match");

  for (auto p : accParallelOp.gangPrivateOperands()) {
    ...
    Value newPrivate =
        outlinedParallelRegion.addWorkgroupAttribution(bufferType);
...
static void analysisLoopOp(acc::LoopOp accLoopOp,
  ...
  if (accLoopOp.isSeq()) {
    ...
    if (auto forOp =
            dyn_cast<scf::ForOp>(accLoopOp.getBody().front().front())) {
...
static Value generateGangValue(OpBuilder &builder, Location loc,
  ...
  // diffUpLb = upper bound - lower bound
...
/// Take value defined in operation or derived value from loops inside the
/// parallel region.
static Value generateNumGangs(acc::ParallelOp parallelOp,
  ...
  OpBuilder builder(parallelOp);
  Location loc = parallelOp.getLoc();
  if (auto numGangs = parallelOp.numGangs())
    return numGangs;
...
static Value generateNumWorkers(acc::ParallelOp parallelOp,
  ...
  OpBuilder builder(parallelOp);
  Location loc = parallelOp.getLoc();
  if (auto numWorkers = parallelOp.numWorkers())
    return numWorkers;
